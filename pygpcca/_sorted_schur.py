...
def _initialize_matrix(M: "petsc4py.PETSc.Mat", P: Union[np.ndarray, spmatrix]) -> None:
    ...
    if issparse(P):
        if not isspmatrix_csr(P):
            warnings.warn("Only CSR sparse matrices are supported, converting.")  # noqa: B028
            P = csr_matrix(P)
...
def sorted_krylov_schur(
    ...
    if nconv < k:
        warnings.warn(
            f"The number of converged eigenpairs is `{nconv}`, but `{k}` were requested."
        )  # noqa: B028
...
def sorted_brandts_schur(P: np.ndarray, k: int, z: str = "LM") -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if np.any(np.array(ap) > 1.0):
        warnings.warn("Reordering of Schur matrix was inaccurate.")  # noqa: B028
...
def sorted_schur(
    ...
    if method == "krylov":
        if petsc4py is None or slepc4py is None:
            ...
            warnings.warn(NO_PETSC_SLEPC_FOUND_MSG)  # noqa: B028
